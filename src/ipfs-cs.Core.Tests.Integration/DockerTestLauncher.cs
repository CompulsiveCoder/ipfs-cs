...
namespace ipfs.Core.Tests.Integration
{
	public class DockerTestLauncher
	{
		...
		private void Launch(Type fixtureType)
		{
			...
			var command = String.Format("cp /ipfs-cs /ipfs-cs-staging -r && cd /ipfs-cs-staging && sh start-integration-test.sh {0} \"{1}\" \"{2}\"", buildMode, assemblyName, fixtureType.FullName);
